...
//------------------------------------------------------------------------------
static const unsigned int g_uiCRC32table[] =	{
...
{
	const unsigned char* pucBuffer = reinterpret_cast<const unsigned char*>(pBuffer);
	uiCRC = uiCRC ^ ~0U;

	while (uiSize--)
		uiCRC = g_uiCRC32table[(uiCRC ^ *pucBuffer++) & 0xFF] ^ (uiCRC >> 8);
