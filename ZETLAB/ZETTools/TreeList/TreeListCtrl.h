...
public:
	...
	BOOL		 MoveItem			 	(HTREEITEM hItem,HTREEITEM hTarget);
	...
	BOOL		 ListMoveItem			(int iRow,int iNewRow);
...
inline	BOOL		 CTreeListCtrl::MoveItem		(HTREEITEM hItem,HTREEITEM hTarget)			{ ASSERT(::IsWindow(m_hWnd)); return TreeList_MoveItem(m_hWnd,hItem,hTarget); }
...
inline	BOOL		 CTreeListCtrl::ListMoveItem		(int iRow,int iNe)						{ return MoveItem        (GetItemOfRow(iRow),GetItemOfRow(iNe));}
