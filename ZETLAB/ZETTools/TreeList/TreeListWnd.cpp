...
typedef struct
	...
	HIMAGELIST	hHdrImg;									// Handle der Icon-Liste für den Header
	...
	ULONGLONG 	uButtonLast;								// Wann wurde eine Maustaste wurde zuletzt gedrückt
...
NMTVDISPINFO	sInfo;
	...
	if(uFlags&TVIF_TEXT)
		...
		uTextSize = sInfo.item.pszText != nullptr ? str_len(sInfo.item.pszText) : 0;
...
NMTVDISPINFO	sInfo;
	...
	if(uFlags&TVIF_TEXT)
		{
		pText		= sInfo.item.pszText;
		uTextSize = sInfo.item.pszText != 0 ? str_len(sInfo.item.pszText) : 0;
		}
...
static int TreeListSetOrderArray(TreeListData *pData, unsigned uItems, unsigned *pArray)
{
	...
	if (!pArray)												// Spezialreihenfolge setzen
	{
		...
		{
			for (uCol = 0; uCol < pData->uColumnCount - 1; uCol++)		// Standartreihenfolge
			{
				aArray[uCol] = uCol;
			}
		}
	...
	ZeroMemory(aFlags, sizeof(aFlags));
...
//*
//*****************************************************************************
//	Verschiebt einen Eintrag. Der Eintrag kann nur innerhalb eines Elternteils
//	verschoben werden.
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages der gelöscht werden soll
//	uTarget		: Ist der Eintrag auf desen Platz der verschobene landen soll (0=am Ende)
//	Ergibt 1 wenn der Eintrag verschoben wurde.
static int TreeListMoveItem(TreeListData *pData,unsigned uItem,unsigned uTarget)
{
BaseItem   *pTarget;
BaseItem   *pParent;
BaseItem   *pEntry;
...
unsigned	uPrev;
	...
	if(uItem==0 || uItem>pData->uTreeItemsMax)				// Ist der Eintrag gültig
		{
		return 0;
		}
	...
	if(uTarget)												// Prüfe das Ziel
		{
		if(uTarget>pData->uTreeItemsMax)return 0;
		pTarget = pData->pTreeItems[uTarget];
		if(!pTarget)return 0;
		if(pTarget->uPrevItem==uItem)
			{
			return 1;
			}
		}
	else{
		if(!pEntry->uNextItem)return 1;						// Eintrag ist bereits am Enden
		pTarget = 0;
		}
	...
	if(uNext)												// Eintrag entfernen
		{
		pNext				= pData ->pTreeItems[uNext];
		pNext->uPrevItem	= uPrev;
		}
	else if(pEntry->uParent)								// Eintrag war das letzte Kind
		{
	...
	else if(pEntry->uParent)								// Ist der Eintrag das erste Kind geworden
		{
		pParent = pData->pTreeItems[pEntry->uParent];
	...
	if(uTarget)												// Zu bestehenten Ziel verschieben
		{
		...
		if(pEntry->uPrevItem)
			{
			pTarget				= pData->pTreeItems[pEntry->uPrevItem];
		...
		if(!uPrev || !pEntry->uPrevItem)					// Beim Elternteil mit Neuberechung beginnen
			{
			uPrev = pEntry->uParent;
			}
		else{
			pNext = pData->pTreeItems[pEntry->uPrevItem];

			if(pPrev->uShowPos>pNext->uShowPos)
				{
				uPrev = pNext->uPrevItem;
				}
			}
		}
	else{													// Eintrag ans Ende verschieben
		if(pEntry->uParent)
			{
			pParent = pData->pTreeItems[pEntry ->uParent   ];
			pTarget	= pData->pTreeItems[pParent->uLastChild];

			pEntry ->uPrevItem  = pParent->uLastChild;
			pEntry ->uNextItem	= 0;
			pTarget->uNextItem	= uItem;
			pParent->uLastChild = uItem;
			}
		else{
			pTarget	= pData->pTreeItems[pData->uLastChild];

			pEntry ->uPrevItem	= pData->uLastChild;
			pEntry ->uNextItem	= 0;
			pTarget->uNextItem	= uItem;
...
static int TreeListDeleteColumn(TreeListData *pData, unsigned uCol)
{
	...
	if (pData->uColumnCount > 0)								// Liste mit Extraeinträgen löschen
	{
		...
		if (pList)
		{
			...
			if (pData->uColumnCount < MAX_COLUMNS - 1)
...
HDC				hDc;
	...
	case TVM_MOVEITEM:											// Einen Eintrag verschieben
			...
			pData = GetHandle(hWnd);
			LOCK(pData);
			lRet  = TreeListMoveItem(pData,U(lParam),U(wParam));
			UNLOCK(pData);
				...
				if(uChange&(TVS_EX_ITEMLINES|TVS_EX_ALWAYSSCROLLY))
					...
					if(uChange&TVS_EX_ALWAYSSCROLLY)
						...
						if(uVal&TVS_EX_ALWAYSSCROLLY)
							...
							UNLOCK(pData);						// WIN-BUG: Die Scrollbar muss an sein für SIF_DISABLENOSCROLL
							SetScrollInfo(pData->hWnd,SB_VERT,&sScroll,TRUE);
							LOCK(pData);
							}
	...
	NextExp:
			if(U(lParam)<=pData->uTreeItemsMax)
				...
				if(pEntry)
					...
					case TVE_TOGGLE:	lRet=(TreeListToggleItem(pData,U(lParam),0))? 0:1;
										break;
						}
					...
					if((wParam&TVE_ALLCHILDS) && pEntry->uFirstChild)
						...
						for(;;)
							...
							if(U(lParam)>pData->uTreeItemsMax)
								...
								pEntry = pData->pTreeItems[U(lParam)];
							if(!pEntry || !pEntry->uNextItem)break;
	...
	case TVM_GETISEARCHSTRING:									// Holt den aktuellen Suchtext
			...
			uDelta = GetTickCount64()-uKeyLast;
...
//******************** Einträge zeichnen **************************************
	...
	for (; uPos < uMax; uPos++)										// Alle Einträge ausgeben
	{
		...
		else if (uPos & 1)											// Farbe wechselweise ändern
		{
	...
	NoRootLines:
		...
		{													// Das Feld speziel zeichnen
			...
			if ((uBits&TVIS_SELECTED) && pData->cHasFocus && uItem == pData->uSelectedItem && !pData->uSelectedSub)
			{
				...
				else if (uBits&TVIS_SELECTED)					// Ist das Feld ohne Focus ausgewählt
				{
					if (pData->cHasFocus)
					{
						uTempColor = GetSysColor(COLOR_HIGHLIGHTTEXT);
			...
			if (sArea.left + pEntry->iTextPixels >= (int)(sArea.right - pData->uScrollX))
			{
				...
				{
					GetTextExtentExPointW(hDc, pText, uTextSize, iSize, &iCount, pPos, &sSize);
				}
