...
{
	// перед использованием функции убедитесь что glGetError возвращает 0
	// иначе функция может не сработать
	//if (Height != NewHight)	// убрано потому что иногда нужно перезагрузить шрифт
	{
		Height = NewHight;
		...
		for (auto &it : m_FontList)
		{
			for (DWORD i = it.first; i < it.second; i++)
			{
				...
				//если подстрочный или надстрочный символ увеличиваем высоту на высоту обычной буквы
...
void FreeTypeFont::PrintTextGL(GLfloat x, GLfloat y, GLfloat z,		/* координаты, (0,0) - нижний левый угол */ 
...
void FreeTypeFont::PrintTextGL(GLfloat x, GLfloat y,				/* координаты, (0,0) - нижний левый угол */ 
						 GLubyte R, GLubyte G, GLubyte B,	/* цвета текста */ 
						 CString Text)			/* отображающаяся строка с заданным форматом */
{
...
CSize FreeTypeFont::GetTextSize(CString Str)
{
	...
	for (auto LineStr = Lines.begin(), End = Lines.end(); LineStr != End; ++LineStr)
	{
		...
		// рассчитываем размер каждого символа отдельно
		for (int n = 0, StrLength = LineStr->GetLength(); n < StrLength; n++)
		{
			auto &it = m_CharsSize.find((*LineStr)[n]);
			if (it != m_CharsSize.end())
				// если этот символ был загружен, то выставляем его сохраненный размер
				CharSize = it->second;
			else
			{
				//Load the Glyph for our character.
		...
		if (Lines.size() > 1)
		{
			// если вычисляем размер многострочного текста
			if (LineStr == --Lines.end())
				// если это последняя строчка то добавляем половину отступа
				TextSize.cy += LONG((GLfloat(Height) / .63f - LineTextSize.cy) / 2);
			else if (LineStr != Lines.begin() && (LineStr != --Lines.end()))
				// если это промежуточная строчка то добавляем ей отступ
