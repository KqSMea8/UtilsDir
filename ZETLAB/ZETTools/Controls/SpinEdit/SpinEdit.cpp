...
T SetNewValBySpinCtrl(_In_ bool bPositiveIncreament, _In_ T CurrentVal)
{
	long Order(0);	// степень числа
	Order = GetOrder(CurrentVal);

	// если число будет уменьшаться
	if ((!bPositiveIncreament && (CurrentVal > 0)) || (bPositiveIncreament && (CurrentVal < 0)))
	{
		T NextVal(CurrentVal);	// получаемое значение на выходе
		if (bPositiveIncreament)
	...
	T Increament = (T)pow((T)10, (T)Order);

	if (!bPositiveIncreament)
		Increament = -Increament;

	return CurrentVal + Increament;
}
...
void CSpinEdit::ReposCtrl(_In_opt_ const CRect &NewRect /*= CRect()*/)
{
	CRect SpinRect;
	if (NewRect.IsRectNull())
	{
		CEditBase::GetWindowRect(SpinRect);
		CEditBase::GetParent()->ScreenToClient(SpinRect);
	}
	else
		SpinRect = NewRect;
...
void CSpinEdit::MoveWindow(int x, int y, int nWidth, int nHeight, BOOL bRepaint /*= TRUE*/)
{
	CRect MoveRect(x, y, x + nWidth, y + nHeight);
	if (m_SpinAlign == SpinEdit::LEFT)
		MoveRect.left += m_SpinWidth;
	else
		MoveRect.right -= m_SpinWidth;
	CEditBase::MoveWindow(MoveRect, bRepaint);
	ReposCtrl(MoveRect);
	CEditBase::RedrawWindow();
}

void CSpinEdit::MoveWindow(LPCRECT lpRect, BOOL bRepaint /*= TRUE*/)
{
	CSpinEdit::MoveWindow(lpRect->left, lpRect->top, lpRect->right - lpRect->left, lpRect->bottom - lpRect->top, bRepaint);
}
