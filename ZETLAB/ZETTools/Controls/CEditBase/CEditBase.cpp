...
LRESULT CEditBase::ShowBuble(_In_ CString sTitle, _In_ CString sText, _In_opt_ INT Icon /*= TTI_ERROR*/)
{
	EDITBALLOONTIP ebt;
...
int CountElements(_In_ CString Str, _In_ wchar_t Char)
{
	int Count(0);
...
BOOL CEditBase::CheckNumericString(_In_ CString Str)
{
	...
	int nKeyCode;
	for (int Ind = Str.GetLength() - 1; Ind >= 0; Ind--)
	{
		nKeyCode = Str[Ind];
		if (nKeyCode < 48 || nKeyCode > 57)	// если вводим не цифры
		{
			switch (nKeyCode)
			{
				case 46:	// точка
				...
				case 45:	// минус					
					...
					if ((m_bUsePositivesDigitsOnly && Ind == 0) || ((Ind != 0) && (Str[Ind - 1] != L'e' && Str[Ind - 1] != L'E')))
						return ShowError();
					break;
