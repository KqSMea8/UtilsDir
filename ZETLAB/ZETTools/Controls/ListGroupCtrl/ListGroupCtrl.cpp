...
int CListGroupCtrl::InsertColumn(_In_ int nCol, _In_ LPCTSTR lpszColumnHeading, _In_opt_ int nFormat /*= -1*/, _In_opt_ int nWidth /*= -1*/,
								 _In_opt_ int nSubItem /*= -1*/, _In_opt_ std::function<int(CString, CString)> SortFunct /*= nullptr*/)
{
...
{
	CheckedList.clear();
	size_t nItemsCount = CListCtrl::GetItemCount();

	if (bCurrentIndexes)
	{
		CheckedList.reserve(nItemsCount);

		for (unsigned Row = 0; Row < nItemsCount; ++Row)
		{
	...
	else
	{
		...
		{
			if (GetCheck(Row) != BST_UNCHECKED)
				CheckedList.push_back(GetItemData(Row));
		}

		for (auto &it : m_DeletedItems)
		{
			if (it.bChecked)
				CheckedList.push_back(it.ItemData.lParam);
		}
	}
}
...
BOOL CListGroupCtrl::PreTranslateMessage(MSG* pMsg)
{
...
void CListGroupCtrl::OnCheckItem()
{
	if (CListCtrl::GetExtendedStyle() & LVS_EX_CHECKBOXES)
	{
...
bool CListGroupCtrl::bIsNeedToRestoreDeletedItem(std::list<DeletedItemsInfo>::iterator Item)
{
	...
	{
		int n;
		for (unsigned Column = 0, Column_Count = Item->ColumnsText.size(); Column < Column_Count; Column++)
		{
			if (Column == 0)
			{
				...
				CListCtrl::SetItemText(n, Column, Item->ColumnsText[Column]);
		}
