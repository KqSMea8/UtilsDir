...
CString CDescriptorFileHelper::FindDescriptorFile( CString &sDirectory, CChannel &channel )
{
...
bool CDescriptorFileHelper::FindChannelInCommonDescriptorFile( CString &sDirectory, CChannel &channel )
{
	return std::find_if(m_mCommonDescriptors[sDirectory].begin(), m_mCommonDescriptors[sDirectory].end(), [&](CChannel &channel_)
	{
...
void CDescriptorFileHelper::AppendCommonDescriptorFile(CString &sDirectory, CString &sCommonDescriptorFile, CChannel &channel, CString &sDescriptorFile, CString &sDataFile/*, CString &sLogFile*/)
{
	xml_document hFile;
...
CString CDescriptorFileHelper::CreateDescriptorFile( CString &sDirectory, CTime tTime, CChannel &channel )
{
	CString sReturn(L"");
	CChannelEx channelex;
	*(CChannel*)(&channelex) = channel;
	...
	FILE* hANP(nullptr);
    if ((zfOpenFile(sFileName, L"wt", &hANP) == 0) && (hANP != nullptr))
	{
...
bool CDescriptorFileHelper::FindChannelInCommonCompressedDescriptorFile( CString &sDirectory, CChannel &channel )
{
	return FindChannelInCommonDescriptorFile(sDirectory, channel);
}

void CDescriptorFileHelper::AppendCommonCompressedDescriptorFile(CString &sDirectory, CString &sCommonDescriptorFile, CChannel &channel, CString &sDescriptorFile, CString &sDataFile, CString &sLogFile)
{
	xml_document hFile;
	if (hFile.load_file(sCommonDescriptorFile, parse_default, encoding_utf8))
	{
