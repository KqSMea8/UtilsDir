...
//	CRect rectText;
	...
	{	
		if( !bDisabled )
		{
			m_ImageList.DrawIndirect( 
				pDC , 
				0 , 
				CPoint( m_RectImage.left + bPushed , m_RectImage.top + bPushed ) , 
		...
		{
			pDC->DrawState( CPoint( m_RectImage.left + bPushed , m_RectImage.top + bPushed ) , 
				CSize( m_bitmap.bmWidth , m_bitmap.bmHeight ) , m_hbmpDisabled , DST_BITMAP | DSS_DISABLED );
		}
	}
	...
	if (ODS_DISABLED & lpDrawItemStruct->itemState)
	{
		...
		pDC->DrawText(strCaption, rectTemp, DT_EDITCONTROL | DT_WORDBREAK | DT_END_ELLIPSIS | DT_CENTER | DT_VCENTER);
...
BOOL CImageButton::SetButtonImage(HICON hIncon, COLORREF clrMask, long ix, long iy)
{
	if (m_bLoaded)
	{
		m_ImageList.DeleteImageList();
	...
	ICONINFO info;
	BOOL bResult = GetIconInfo(hIncon, &info);
	if (bResult != FALSE)
	{
		if (m_ImageList.Create(ix, iy, ILC_COLOR32, 1, 1))
		{
			m_ImageList.Add(hIncon);
			m_hIcon = hIncon;
			m_clrMask = clrMask;
			m_bLoaded = TRUE;
...
void CImageButton::SetImageOffset( long lOffset )
{
	m_lOffset = lOffset;
}

void CImageButton::SetImageIndent( long lIndent )
{
	m_lIndent = lIndent;
}

void CImageButton::SetImageRect(const CRect &rcImage)
{
	m_RectImage = rcImage;
}
...
void CImageButton::ReCalculateSettings()
{
	...
	//Default Left Top
	m_RectImage.top = m_lOffset;
	m_RectImage.bottom = m_RectImage.top + m_bitmap.bmHeight;

	m_RectImage.left = m_lOffset;
	m_RectImage.right = m_bitmap.bmWidth + m_lOffset;

	//Image Right 
	if( m_dwAlign & IMAGE_RIGHT ) 
	{
		m_RectImage.left = (rectClient.right - m_bitmap.bmWidth) - m_lOffset;
	...
	{
		m_RectImage.top = rectClient.Height() - m_bitmap.bmHeight - m_lOffset;
		if (m_RectImage.top < m_lOffset)
			m_RectImage.top = m_lOffset;
	...
	if( m_dwAlign & TEXT_INCLUDE ) 
	{  //Including Text
		if( m_dwAlign & IMAGE_VCENTER ) 
		{
			m_RectText.top = m_lOffset;
			m_RectText.bottom = rectClient.bottom - m_lOffset;
			//Calculate Text Focus Rect
			m_RectTextFocus.top = m_lIndent;
			m_RectTextFocus.bottom = rectClient.bottom - m_lIndent;
		}
		else 
		{
			...
			{
				m_RectText.top = m_lOffset;
				m_RectText.bottom = m_RectImage.top - m_lOffset;
				//Calculate Text Focus Rect
				m_RectTextFocus.top = m_lIndent;
				m_RectTextFocus.bottom = m_RectImage.top - m_lIndent;
			}
			else 
			{
				m_RectText.top = m_RectImage.bottom + m_lOffset;
				m_RectText.bottom = rectClient.bottom - m_lOffset;
				//Calculate Text Focus Rect
				m_RectTextFocus.top = m_RectImage.bottom + m_lIndent;
				m_RectTextFocus.bottom = rectClient.bottom - m_lIndent;
			}
		}
		//Image Center
		if( m_dwAlign & IMAGE_VCENTER ) 
		{
			if( m_dwAlign & IMAGE_RIGHT ) 
			{
				m_RectText.left = m_lOffset;
				m_RectText.right = m_RectImage.left - m_lOffset;
				//Calculate Text Focus Rect
				m_RectTextFocus.left = m_lIndent;
				m_RectTextFocus.right = m_RectImage.left - m_lIndent;
			}
			else 
			{
				m_RectText.left = m_RectImage.right + m_lOffset;
				m_RectText.right = rectClient.right - m_lOffset;
				//Calculate Text Focus Rect
				m_RectTextFocus.left = m_RectImage.right + m_lIndent;
				m_RectTextFocus.right = rectClient.right - m_lIndent;
			}
		}
		else 
		{
...
void CImageButton::SetTextFontSize(long lSize)
{
	m_lFontSize = lSize;
	...
	log.lfHeight = -m_lFontSize;
	m_Font.CreateFontIndirect(&log);
}

void CImageButton::SetTextRect(const CRect &rcText)
{
	m_RectText = rcText;
}
