...
public:
	...
	void SetTextFontSize( long lSize );
	void SetTextRect(const CRect &rcText );
	void SetImageOffset( long lOffset );
	void SetImageIndent( long lIndent );
	void SetImageRect(const CRect &rcImage );
	
	BOOL SetAlignStyle( DWORD dwStyle );
	BOOL SetButtonImage( UINT uiImageID , COLORREF  clrMask );
	BOOL SetButtonImage( HICON hIncon, COLORREF  clrMask, long ix, long iy );
