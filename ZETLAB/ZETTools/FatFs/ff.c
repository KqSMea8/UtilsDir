...
)
{
	...
	if (vol < _VOLUMES) 		/* Found a drive number, get and strip it */
	{
		UINT len = _tcslen(p);
		UINT i;

		for(i = 1; i < min(4, len); i += 1)	/*4 - count digits in _VOLUMES + 1, по-хорошему нужно считать логарифм, но на первое время так сойдет*/
		{
			if(p[i] == ':')
			{
				pos = i;
	...
	if(pos)
	{
		p += pos + 1; *path = p;				/* Return pointer to the path name */
	}
