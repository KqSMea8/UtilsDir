...
//       You can try it, but itґs not tested.
...
public:
  ...
  static BOOL CALLBACK InitDefaultControl(HWND hWnd, LPARAM lParam) {
	...
	// donґt add the child-window if itґs not an immediate child
  ...
  void Initialize(HWND hWndParent, DWORD dwFlags/*=0*/) {
	...
	if (dwFlags & ANIF_SIZEGRIP) {
	  ...
	  m_hWndSizeGrip  = ::CreateWindow(L"ScrollBar", L"", WS_CHILD | WS_VISIBLE | SBS_SIZEGRIP, m_rcClient.right-sz1.cx, m_rcClient.bottom-sz1.cy, sz1.cx, sz1.cy, m_hWndParent, NULL, NULL, 0);
	...
	for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
	  ...
	  if (m_Ctrl[iCtrl].nFlags==ANF_AUTOMATIC) {
		...
		if (m_Ctrl[iCtrl].rect.top   <   (double)(szClient.cy/2)) m_Ctrl[iCtrl].nFlags|=ANF_TOP;//исправлена из-за исправления ошибки найденной 2014.04.16 pvs-studio

		if (m_Ctrl[iCtrl].rect.bottom >= (double)(szClient.cy/2)) m_Ctrl[iCtrl].nFlags|=ANF_BOTTOM;//исправлена из-за исправления ошибки найденной 2014.04.16 pvs-studio
		...
		if (m_Ctrl[iCtrl].rect.left  <  (double)(szClient.cx/2)) m_Ctrl[iCtrl].nFlags|=ANF_LEFT; //исправлена из-за исправления ошибки найденной 2014.04.16 pvs-studio
		if (m_Ctrl[iCtrl].rect.right >= (double)(szClient.cx/2)) m_Ctrl[iCtrl].nFlags|=ANF_RIGHT;//исправлена из-за исправления ошибки найденной 2014.04.16 pvs-studio
  ...
  void HandleAnchors(RECT *pRectParent) {
	...
	for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
	  ...
	  // weґve nothing changed until now
	  bChanged = FALSE;
