...
//------------------------------------------------------------------------------
int g_nMainParam = 0;		// счЄтчик
//------------------------------------------------------------------------------
#ifdef _GDIPLUS
CDMainParam::CDMainParam(CRect rect, PVOID ppGraphics,
	bool *pFlagIsChange, double *pTimeAnimation) :
	CCustomDraw(rect, ppGraphics, pFlagIsChange, pTimeAnimation)
#else
CDMainParam::CDMainParam(CRect rect,
...
#endif
{
	...
	m_param.nElement = ++g_nMainParam;
	m_param.Name = L"MainParam";
	//TRACE("CREATE CDMainParam %s\n", (CStringA)m_pParamDraw->Name);
...
#ifdef _ACTIVEX_ZIS
	//центр
	m_pMain[0] = new CDetail(long(24), (m_rectForDraw.right + m_rectForDraw.left - Bolder) / 2.0, (m_rectForDraw.bottom + m_rectForDraw.top) / 2.0, -Depth, 0, m_rectForDraw.Width() - Bolder, m_rectForDraw.Height() - 2 * Bolder, Depth);
	//верхн€€ часть панели
	m_pMain[1] = new CDetail(long(17), (m_rectForDraw.right + m_rectForDraw.left - Bolder) / 2.0, Bolder / 2.0 + m_rectForDraw.top, -Depth, 0, m_rectForDraw.Width() - Bolder, Bolder, Depth);
	//правый верхний угол
	m_pMain[2] = new CDetail(long(18), m_rectForDraw.right - Bolder / 2.0, Bolder / 2.0 + m_rectForDraw.top, -Depth, 0, Bolder, Bolder, Depth);
	//права€ часть панели
	m_pMain[3] = new CDetail(long(19), m_rectForDraw.right - Bolder / 2.0, (m_rectForDraw.bottom + m_rect.top) / 2.0, -Depth, 0, Bolder, m_rectForDraw.Height() - 2 * Bolder, Depth);
	//правый нижний угол
	m_pMain[4] = new CDetail(long(20), m_rectForDraw.right - Bolder / 2.0, m_rectForDraw.bottom - Bolder / 2.0, -Depth, 0, Bolder, Bolder, Depth);
	//нижн€€ часть панели
	m_pMain[5] = new CDetail(long(21), (m_rectForDraw.right + m_rect.left - Bolder) / 2.0, m_rectForDraw.bottom - Bolder / 2.0, -Depth, 0, m_rectForDraw.Width() - Bolder, Bolder, Depth);//*/
#else
	//центр
	m_pMain[0] = new CDetail("C:\\ZETTools\\Interface2012\\Models\\ѕанель\\центр_панелька.stl",
		(m_rect.right + m_rect.left - Bolder)/2, (m_rect.bottom + m_rect.top)/2, -Depth, 0, 
		m_rect.Width() - Bolder, m_rect.Height() - 2*Bolder, Depth);
	//верхн€€ часть панели
	m_pMain[1] = new CDetail("C:\\ZETTools\\Interface2012\\Models\\ѕанель\\низ_панелька.stl",
		(m_rect.right + m_rect.left - Bolder)/2, Bolder/2 + m_rect.top, -Depth, 0, 
		m_rect.Width() - Bolder, Bolder, Depth);
	//правый верхний угол
	m_pMain[2] = new CDetail("C:\\ZETTools\\Interface2012\\Models\\ѕанель\\правый нижний _панелька.stl",
		m_rect.right - Bolder/2, Bolder/2 + m_rect.top, -Depth, 0,
		Bolder, Bolder, Depth);	
	//права€ часть панели
	m_pMain[3] = new CDetail("C:\\ZETTools\\Interface2012\\Models\\ѕанель\\права€_панелька.stl",
		m_rect.right - Bolder/2,(m_rect.bottom + m_rect.top)/2, -Depth, 0, 
		Bolder, m_rect.Height() - 2*Bolder, Depth);
	//правый нижний угол
	m_pMain[4] = new CDetail("C:\\ZETTools\\Interface2012\\Models\\ѕанель\\правый верхний _панелька.stl",
		m_rect.right - Bolder/2, m_rect.bottom - Bolder/2, -Depth, 0, 
		Bolder, Bolder, Depth);
	//нижн€€ часть панели
	m_pMain[5] = new CDetail("C:\\ZETTools\\Interface2012\\Models\\ѕанель\\верх_панелька.stl",
		(m_rect.right + m_rect.left - Bolder)/2, m_rect.bottom - Bolder/2, -Depth, 0, 
		m_rect.Width() - Bolder, Bolder, Depth);	//*/
#endif
...
//------------------------------------------------------------------------------
CDMainParam::~CDMainParam()
{
	//TRACE("DELETE CDMainParam %s\n", (CStringA)m_pParamDraw->Name);
#ifdef _GDIPLUS
...
//------------------------------------------------------------------------------
void CDMainParam::SetTitle(CString str)
{
...
//------------------------------------------------------------------------------
void CDMainParam::SetImageFileName(CString str)
{
...
//------------------------------------------------------------------------------
void CDMainParam::SetWidthBorder(int val)
{
...
//------------------------------------------------------------------------------
void CDMainParam::SetColorBorderStart(COLORREF val)
{
...
//------------------------------------------------------------------------------
void CDMainParam::SetColorBorderEnd(COLORREF val)
{
...
//------------------------------------------------------------------------------
void CDMainParam::SetSizeIcon(int val)
{
...
#ifdef _GDIPLUS
//------------------------------------------------------------------------------
void CDMainParam::SetGradientMode(LinearGradientMode val)
{
...
//------------------------------------------------------------------------------
void CDMainParam::OnDraw()
{// √лавный контейнер всегда отображаетс€, поэтому без if (m_param.bVisible)
...
{// √лавный контейнер всегда отображаетс€, поэтому без if (m_param.bVisible)
	...
	glPopAttrib();
}	// берЄтс€ из ресурсов, о которых данный класс не имеет права ничего знать
//------------------------------------------------------------------------------
void CDMainParam::SaveParameters(SimpleXML *pSimpleXML)
{
    CString cString;
    char String[16];
    ...
    hXml hMainPanelRoot = pSimpleXML->AddElement(L"MainParam_"+cString, hInterfaceRoot);
...
//------------------------------------------------------------------------------
void CDMainParam::LoadParameters(SimpleXML *pSimpleXML)
{
    VARIANT vAtrrib;
    CString str = L"";
    CString Pattern = L"MainParam_";
    char String[16];
...
//------------------------------------------------------------------------------
void CDMainParam::OffDraw()
{
	//SwapBuffers(wglGetCurrentDC());
}
//------------------------------------------------------------------------------
void CDMainParam::SetColor(GLfloat *ma, GLfloat *md,	
	GLfloat *me, 
