...
{
    int pack_count = (int)ceil(((double)sat_count) / NMEA_MAXSAT_GSV);
...
void nmea_info2GPGSV(const nmeaINFO *info, nmeaGPGSV *pack, int pack_idx)
{
    ...
    for(pit = 0, sit = pack->pack_index * NMEA_MAXSAT_GSV; pit < NMEA_MAXSAT_GSV; ++pit, ++sit)
    {
        pack->id[pit] = info->satinfo.sat[sit].id;
        pack->elv[pit] = info->satinfo.sat[sit].elv;
        pack->azimuth[pit] = info->satinfo.sat[sit].azimuth;
        pack->sig[pit] = info->satinfo.sat[sit].sig;
    }
}
